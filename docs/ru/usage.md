...
Если включен SQLite, в настройках клиента также есть optional traffic limit в GiB. Пустое значение означает без лимита. Когда записанный трафик достигает лимита, awg-forge отключает клиента, а строка клиента показывает `лимит исчерпан`. Увеличь или очисти лимит, затем включи клиента снова.
