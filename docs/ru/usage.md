...
Если включен SQLite, в настройках клиента также есть optional traffic limit в GiB. Пустое значение означает без лимита. Когда записанный трафик достигает лимита, awg-forge отключает клиента, а строка клиента показывает `лимит исчерпан`. Попытка включить клиента отклоняется, пока записанный total остается выше лимита. Увеличь или очисти лимит, затем включи клиента снова.
