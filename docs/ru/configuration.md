...
В настройках клиента можно сохранить optional traffic limit в GiB, если включен SQLite. Лимит показывается как `итого / лимит`; пустое значение означает без лимита. Когда записанный трафик достигает или превышает лимит, awg-forge отключает клиента через обычный render/apply path и пишет audit event. Попытки включить клиента отклоняются, пока записанный total остается выше лимита, поэтому сначала увеличь или очисти лимит. HTTP API возвращает `409 Conflict`; CLI возвращает ошибку.
