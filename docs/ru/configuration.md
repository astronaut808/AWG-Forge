...
## Bootstrap первого туннеля

В новых установках `.env` хранит только runtime-настройки запуска, а настройки туннелей живут в `state.json`.

При чистой установке `install.sh` записывает запрос на создание первого туннеля в `data/bootstrap.env`. При первом старте awg-forge читает этот файл, создает `state.json`, рендерит конфиги и удаляет `bootstrap.env`. После этого туннели управляются через Web UI/API и сохраняются в `state.json`.
...
В новых установках это значение попадает в `state.json` при первом bootstrap. Изменение `SERVER_HOST` в `.env` после создания state не переписывает существующие туннели.
