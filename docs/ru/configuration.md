...
- `AWG3_EXPERIMENTAL`: включает экспериментальный профиль AmneziaWG 3.0. По умолчанию `false`; включай его только с соответствующим экспериментальным образом из [матрицы протоколов](../protocol-matrix.md).
...
Установщик намеренно не предлагает AWG 3.0. AWG 2.0 остаётся поддерживаемым профилем по умолчанию.
...
`tls use environment` сначала проверяет environment, а затем удаляет managed override. `tls status` показывает configured settings; `Maintenance` -> `Support` показывает TLS runtime текущего процесса.
