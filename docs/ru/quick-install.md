...
При чистой установке скрипт также предлагает ACME-сертификат для домена. Выбирай его только после того, как домен направлен на этот сервер, а TCP/80 доступен из Интернета. Панель остаётся на выбранном Web UI-порту; ограничения и настройка существующей установки описаны в [TLS-конфигурации](configuration.md#tls-для-web-ui).
