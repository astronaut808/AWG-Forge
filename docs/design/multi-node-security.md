...
| Cloned node data directory | Detect simultaneous identity use; require explicit identity replacement; change `state_epoch`; revoke old certificate | Full clone has the old private key until controller revocation is observed |
