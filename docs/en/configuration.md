...
Client creation and client settings can store an optional traffic limit when SQLite is enabled. The Web UI accepts MiB, GiB, or TiB and displays the limit as `total / limit`; unlimited means no limit row is stored. When recorded traffic reaches or exceeds the configured limit, awg-forge disables the client through the normal render/apply path and writes an audit event. Re-enable attempts are rejected while the recorded total is still over the limit, so increase or clear the limit before enabling the client. The HTTP API returns `409 Conflict`; the CLI returns an error.
