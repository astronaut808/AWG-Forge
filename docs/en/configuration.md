...
Client settings can store an optional traffic limit in GiB when SQLite is enabled. The limit is displayed as `total / limit`; an empty limit means unlimited. When recorded traffic reaches or exceeds the configured limit, awg-forge disables the client through the normal render/apply path and writes an audit event. Re-enable the client manually after increasing or clearing the limit.
